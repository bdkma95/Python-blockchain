...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, recipient = 'Kirill', amount = 1)
...
def is_valid():
    ...
    if is_valid:
        ...
        response = {'We have a problem. The blockchain is not valid!'}
    return jsonify(response), 200
...
def connect_node():
    ...
    response = {'message': 'All the nodes are now connected. The Bcoin blockchain now contains the following nodes:',
...
def replace_chain():
    is_chain_replaced = blockchain.replace.chain()
